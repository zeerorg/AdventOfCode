...
func Main10() {
	_tempStars := strings.Split(Day10Data, "\n")
	stars := make([]Star, 0)
	for _, _tempStar := range _tempStars {
		stars = append(stars, GetStar(_tempStar))
	}
	minX, minY, maxX, maxY := GetBounds(stars)
	minWidth := maxX - minX
	minHeight := maxY - minY
	steps := 0
	...
	for minWidth > 270 {
		...
		//reader.ReadRune()
		minX, minY, maxX, maxY = GetBounds(stars)
		minWidth = GetMin(minWidth, maxX-minX)
		minHeight = GetMin(minHeight, maxY-minY)
		stars = TakeStep(stars)
		steps += 1
	}

	for minWidth != 260 {
		PrintGraph(stars, minX, minY, maxX, maxY)
		minX, minY, maxX, maxY = GetBounds(stars)
		minWidth = GetMin(minWidth, maxX-minX)
		minHeight = GetMin(minHeight, maxY-minY)
		steps += 1
	...
	PrintGraph(stars, minX, minY, maxX, maxY)
	fmt.Println(minWidth, minHeight)
	//fmt.Println("Yo 10")
}

func GetBounds(stars []Star) (int, int, int, int) {
	maxX := stars[0].Pos.X
	maxY := stars[0].Pos.Y
	minX := 0
	minY := 0
	for _, star := range stars {
		maxX = GetMax(maxX, star.Pos.X)
		maxY = GetMax(maxY, star.Pos.Y)
		minY = GetMin(minY, star.Pos.Y)
		minX = GetMin(minX, star.Pos.X)
	}
	return minX, minY, maxX, maxY
}

func PrintGraph(stars []Star, minX, minY, maxX, maxY int) {
	sky := make(map[Point10]bool)
	for _, star := range stars {
		sky[star.Pos] = true
	}
...
func GetStar(data string) Star {
	_tempPos := strings.Split(data, " vel")[0]
	_tempVel := strings.Split(data, " vel")[1]
	_tempPos = strings.Split(_tempPos, "<")[1]
	_tempVel = strings.Split(_tempVel, "<")[1]
	_tempPos = _tempPos[:len(_tempPos)-1]
	_tempVel = _tempVel[:len(_tempVel)-1]

	position := Point10{
		X: StrToInt(strings.Trim(strings.Split(_tempPos, ",")[0], " ")),
		Y: StrToInt(strings.Trim(strings.Split(_tempPos, ",")[1], " ")),
	}

	velocity := Point10{
		X: StrToInt(strings.Trim(strings.Split(_tempVel, ",")[0], " ")),
		Y: StrToInt(strings.Trim(strings.Split(_tempVel, ",")[1], " ")),
	}
...
const Day10Data = `position=<-31138, -10302> velocity=< 3,  1>
